 // Default configuration
      const defaultConfig = {
        system_title: "Hệ Thống Quản L�� Camera",
      ...
      function generateCameraGrid() {
        ...
        grid.innerHTML = cameras
          .map(
            (cam, index) => `
        <div class="camera-card bg-white rounded-xl overflow-hidden border ${cam.status === "alert" ? "camera-alert border-gray-200" : cam.status === "offline" ? "border-gray-300" : "border-gray-200"} cursor-pointer">
          <div class="camera-feed aspect-video relative ${cam.status === "offline" ? "opacity-60" : ""}">
            ...
            <div class="absolute inset-0 flex items-center justify-center ${cam.status === "offline" ? "z-20" : ""}">
              ${
                cam.status === "offline"
                  ? `
              ...
              `
                  : `
                <svg class="w-12 h-12 text-gray-600 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      async function onConfigChange(config) {
        ...
        document.getElementById("warehouse-title").textContent =
          config.warehouse_name || defaultConfig.warehouse_name;
      }
